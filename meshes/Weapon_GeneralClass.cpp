...
void AWeapon_GeneralClass::DoDamage(AActor* Actor) {
	return;
}
...
void AWeapon_GeneralClass::PlaySpawnAnimation() {
	...
	if (AnimInstance) {
		if (IsValid(AnimMontage_SpawnEffect)) {
			...
			AnimInstance->OnMontageEnded.AddUniqueDynamic(this, &AWeapon_GeneralClass::OnSpawnMontageEnded);
		}
	}
}
...
void AWeapon_GeneralClass::PlayEndAnimation() {
	...
	if (AnimInstance) {
		if (IsValid(AnimMontage_DieEffect)) {
			...
			// Bind the OnMontageEnded event to a custom function
			AnimInstance->OnMontageEnded.AddUniqueDynamic(this, &AWeapon_GeneralClass::OnDieMontageEnded);
		}
		else {
			Destroy();
		}
