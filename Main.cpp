...
AMain::AMain()
{
	...
	AbilitySystem = CreateDefaultSubobject<AAbilitySystem>(TEXT("Ability System"));
...
void AMain::BeginPlay()
{
	...
	/*
	SetAbility(0, EAbilityCode::EShotgun);
	SetAbility(1, EAbilityCode::EBlackHoleBomb);
	SetAbility(2, EAbilityCode::EFForward);
	SetAbility(3, EAbilityCode::ETeleport);
	SetAbility(4, EAbilityCode::EBulletTime);
	*/
...
// give out the main stats for post game showcase
void AMain::SetStatsToGameInstance()
{
	UTrueGame2Instance* TrueGameInstance = Cast<UTrueGame2Instance>(UGameplayStatics::GetGameInstance(GetWorld()));
	if (TrueGameInstance)
	{
		TrueGameInstance->SetTotalDamageDealed(TotalDamage);
		TrueGameInstance->SetTotalEnemiesKilled(TotalEnemiesKilled);
		TrueGameInstance->SetTotalXPLeanrt(XPComponent->GetXP());
		TrueGameInstance->SetTotalDamageTaken(TotalDamageTaken);
		TrueGameInstance->SetTotalItemsCollected(AllArtifactList.Num());
		TrueGameInstance->SetAllArtifactList(AllArtifactList);
	}
}
...
void AMain::SetNewDetectionAreaRadius(float NewScale)
{
	...
	DrawDebugSphere
	(
		GetWorld(), GetActorLocation(), NewDetectionAreaRadius, 10, FColor::Blue, false, 10.0f, 2, 10.0f
	);
}
...
TArray<AActor*> AMain::GetAllDetectedActors()
{
	...
	for (AActor* Actor : ActorsDetected)
	{
		DrawDebugSphere
		(
			GetWorld(), Actor->GetActorLocation(), 100.0f, 10, FColor::Green, false, 10.0f, 2, 1.0f
		);
	}
...
void AMain::UpdateDisplayVariables()
{
...
void AMain::SetAbility(int slot, EAbilityCode abilityCode)
{
	if (AbilitySystem->CheckAbility(slot))
	{
		...
		case 0:
		{
			/*case 1 should be treat different, because it cause:
			*	New idle, run, walk, jump? animation
			*	Attach a weapon with the character
			*/
...
void AMain::UseBasicAttack()
{
	...
	if (AbilitiesChangeInput.Contains(CurrentAbilityInHand))
	{
		...
		{	
			...
			default:
			{
				GEngine->AddOnScreenDebugMessage(-1, 10.0f, FColor::Yellow, TEXT("SOMETHING IS DEFINITLY WRONG"));
				break;
			}
		}		
...
void AMain::PlayMontages(EAbilityCode abilityCode, float PlayRate)
{
	...
	switch (abilityCode)
	{
		...
		case EAbilityCode::ETeleport:
		{
			...
			if (AbilitySystem->GetTPAbility()->GetIsAbleToTP())
			{
			...
			else if (AbilitySystem->GetTPAbility()->GetIsAbleToPutMark())
			{
				if (IsValid(Teleport_PutMark_Montage))
				{
					PlaySelectedMontage(Teleport_PutMark_Montage, PlayRate);
				}
			}
			break;
		}
...
void AMain::PlaySelectedMontage(UAnimMontage* ChosenMontage, float PlayRate) {
	...
	if (AnimInstance) {
		if (IsValid(ChosenMontage)) {
			...
			AnimInstance->OnMontageEnded.AddDynamic(this, &AMain::OnMontageEnded);
		}
	}
}
...
void AMain::ProcessAbilityFeedBacks() {
	if (FeedBackContainer->GetExistance()) {
		...
		//UpdateMoveSpeedBoost(FeedBackContainer->GetMoveSpeedReward());
		//UpdateMoveSpeed(MoveSpeedBoost);
...
void AMain::ReleaseBHBomb() {
	if (Weapon_BlackHoleBomb != nullptr) {
		...
		DrawDebugSphere
		(
			GetWorld(), StartPoint, 1.0f, 10, FColor::Green, false, 10.0f, 2, 10.0f
		);
		DrawDebugSphere
		(
			GetWorld(), BombDestination, 1.0f, 10, FColor::Red, false, 10.0f, 2, 10.0f
		);
		...
		Weapon_BlackHoleBomb->SetbGravityFire(1200.0f, true);
